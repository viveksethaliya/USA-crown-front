...
export default function BrandsPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) { toast.success('Brand deleted successfully'); fetchBrands(); }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-[#312f2c]">Brands</h2>
          <p className="text-[#312f2c]/55 text-sm mt-1">Manage product manufacturers and brands</p>
        </div>
        <button
      ...
      {showForm && (
        <div className="bg-[#ece9e1] border border-[#312f2c]/10 p-6 rounded-xl relative overflow-hidden">
          ...
          <h3 className="text-lg font-medium text-[#312f2c] mb-4">{editingId ? 'Edit Brand' : 'Create New Brand'}</h3>
          <form onSubmit={handleSave} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-[#312f2c]/65 mb-1">Brand Name</label>
                <input
                  type="text"
                  value={formData.name}
