...
// ─── Alt Text Modal ──────────────────────────────────────────────────────────
// Isolated into its own component so that typing in the input ONLY re-renders
// this modal — not the parent page and its entire image grid.
interface AltTextModalProps {
  file: MediaFile;
  onClose: () => void;
  onSave: (file: MediaFile, altText: string) => Promise<void>;
}

const AltTextModal = memo(function AltTextModal({ file, onClose, onSave }: AltTextModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#312f2c]/70 p-4">
      <div className="bg-[#f0ede5] border border-[#312f2c]/12 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        <div className="p-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={onClose} className="text-[#312f2c]/40 hover:text-[#312f2c] transition-colors">
              <X className="w-5 h-5" />
            </button>
          </div>
          <div className="mb-4">
            <p className="text-sm text-[#312f2c]/50 mb-2 truncate">File: {file.name}</p>
            <div className="aspect-video bg-[#312f2c]/5 rounded-lg overflow-hidden flex items-center justify-center mb-4 border border-[#312f2c]/10">
              <img src={file.url} alt="Preview" className="object-contain w-full h-full" />
            </div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              onClick={handleSave}
              disabled={isSaving}
              className="px-4 py-2 bg-[#312f2c] hover:bg-[#312f2c]/85 text-[#f0ede5] rounded-lg transition-colors disabled:opacity-50 flex items-center gap-2"
            >
...
// ─── Grid Item ───────────────────────────────────────────────────────────────
// Memoized so it only re-renders when its own file data changes —
// NOT when unrelated state (altText, editingAltFile) changes in the parent.
interface GridItemProps {
  file: MediaFile;
  formatBytes: (bytes: number) => string;
  onEditAlt: (file: MediaFile) => void;
  onCopy: (url: string) => void;
  onDelete: (file: MediaFile) => void;
}

const GridItem = memo(function GridItem({ file, formatBytes, onEditAlt, onCopy, onDelete }: GridItemProps) {
  const isImage = file.metadata?.mimetype?.startsWith('image/');
  const size = formatBytes(file.metadata?.size || 0);

  return (
    <div className="group relative bg-[#ece9e1] border border-[#312f2c]/10 rounded-xl overflow-hidden hover:border-[#d1a054]/40 hover:shadow-sm transition-all flex flex-col">
      <div className="aspect-square bg-[#312f2c]/5 flex items-center justify-center relative overflow-hidden">
        {isImage ? (
          <img src={file.url} alt={file.alt_text || file.name} className="object-cover w-full h-full opacity-90 group-hover:opacity-100 transition-opacity" loading="lazy" />
        ) : (
          <FileText className="w-12 h-12 text-[#312f2c]/30 group-hover:text-[#312f2c]/50 transition-colors" />
        )}
        {/* Actions overlay — solid bg, no backdrop-blur */}
        <div className="absolute inset-0 bg-[#312f2c]/75 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
          {isImage && (
            <button
              onClick={() => onEditAlt(file)}
          ...
          <button onClick={() => onCopy(file.url)} className="p-2 bg-white/15 hover:bg-[#d1a054]/80 text-white rounded-lg transition-colors" title="Copy URL">
            <Copy className="w-4 h-4" />
          </button>
          <a href={file.url} target="_blank" rel="noopener noreferrer" className="p-2 bg-white/15 hover:bg-white/25 text-white rounded-lg transition-colors" title="Open in new tab">
            <ExternalLink className="w-4 h-4" />
          </a>
          <button onClick={() => onDelete(file)} className="p-2 bg-white/15 hover:bg-red-600 text-white rounded-lg transition-colors" title="Delete">
...
export default function MediaLibraryPage() {
  ...
  // Stable callback — wrapped in useCallback so GridItem doesn't re-render when
  // unrelated parent state changes.
  const handleSaveAltText = useCallback(async (file: MediaFile, altText: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setFiles(prev => prev.map(f => f.id === file.id ? { ...f, alt_text: altText } : f));
        setEditingAltFile(null);
        toast.success('Alt text saved successfully');
        // Re-fetch in background to confirm persisted value
        fetchMedia();
      } else {
        let errMsg = 'Failed to save alt text';
  ...
  const handleEditAlt = useCallback((file: MediaFile) => {
    setEditingAltFile(file);
  }, []);

  const handleCloseModal = useCallback(() => setEditingAltFile(null), []);
  ...
  const quotaLimitBytes = 1 * 1024 * 1024 * 1024;
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="bg-[#ece9e1] border border-[#312f2c]/10 rounded-xl p-12 text-center">
          ...
          <button onClick={() => fileInputRef.current?.click()} className="px-6 py-2 bg-[#312f2c] hover:bg-[#312f2c]/85 text-[#f0ede5] rounded-lg transition-colors inline-flex items-center gap-2">
      ...
      {/* Alt Text Modal — isolated so typing only re-renders the modal */}
      {editingAltFile && (
        <AltTextModal
          file={editingAltFile}
